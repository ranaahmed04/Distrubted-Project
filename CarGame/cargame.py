...
class CarRacing:
    ...
    def initialize(self):
        self.crashed = False
        #client.send("Player1 done his loading".encode('utf-8'))
            ...
            for event in pygame.event.get():
                ...
                if (event.type == pygame.KEYDOWN):
                    if (event.key == pygame.K_LEFT):
                        with lock:
                            client.send((f'{PlayerTitle} Go left').encode('utf-8'))
                            print(f'{PlayerTitle} Go left')
                        self.car_x_coordinate -= 50
                        print ("CAR X COORDINATES: %s" % self.car_x_coordinate)
                    if (event.key == pygame.K_RIGHT):
                        with lock:
                            client.send(f'{PlayerTitle} Go Right'.encode('utf-8'))
                        self.car_x_coordinate += 50
                        print ("CAR X COORDINATES: %s" % self.car_x_coordinate)
